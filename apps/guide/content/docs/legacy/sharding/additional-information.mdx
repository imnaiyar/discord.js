...
<Callout>
	<DocsLink path="ShardClientUtil:Class#ids">`Client#shard`</DocsLink> can hold multiple ids. If you use the default
	sharding manager, the `.ids` array will only have one entry.
</Callout>
