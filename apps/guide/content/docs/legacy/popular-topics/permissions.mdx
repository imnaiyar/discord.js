...
There are two utility methods to easily determine the final permissions for a guild member or role in a specific channel: `GuildChannel#permissionsFor` and `GuildMember#permissionsIn` - `Role#permissionsIn`. All return a <DocsLink path="PermissionsBitField:Class" /> object.
...
You can also use this approach for other <DocsLink path="PermissionResolvable:TypeAlias" /> like flag arrays or flags.
