...
Let's start by glancing at the <DocsLink path="Guild:Class#fetchAuditLogs" type="method" /> method and how to work with it. Like many discord.js methods, it returns a [`Promise`](../additional-info/async-await) containing the <DocsLink path="GuildAuditLogs:Class" /> object. This object has one property, `entries`, which holds a [`Collection`](../additional-info/collections) of <DocsLink path="GuildAuditLogsEntry:Class" /> objects, and consequently, the information you want to retrieve.
