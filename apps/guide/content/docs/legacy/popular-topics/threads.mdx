...
<Callout>You can use the `ThreadChannel#isThread` type guard to make sure a channel is a <DocsLink path="ThreadChannel:Class" />!</Callout>
...
- <DocsLink path="Client:Class#threadMembersUpdate" />: Emitted whenever members are added or removed from a thread. Requires `GuildMembers` privileged intent.
