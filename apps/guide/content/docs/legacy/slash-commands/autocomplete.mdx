...
To handle an <DocsLink path="AutocompleteInteraction:Class"/>, use the `BaseInteraction#isAutocomplete` type guard to make sure the interaction instance is an autocomplete interaction. You can do this in a separate `interactionCreate` listener:
...
The <DocsLink path="AutocompleteInteraction:Class"/> class provides the `AutocompleteInteraction#respond` method to send a response. Using this, you can submit an array of <DocsLink path="ApplicationCommandOptionChoiceData:Interface" /> objects for the user to choose from. Passing an empty array will show "No options match your search" for the user.
