...
import * as threadOnlyChannel from "./ThreadOnlyChannel.js";
...
'use strict';
const { ChannelFlags, ChannelType, PermissionFlagsBits, Routes } = v10;
const getThreadOnlyChannel = lazy(() => threadOnlyChannel);
/**
...
class ThreadChannel extends BaseChannel {
     ...
     */
    get editable() {
        return ((this.ownerId === this.client.user.id && (this.type !== ChannelType.PrivateThread || this.joined)) ||
            this.manageable);
    }
    /**
    ...
    get joinable() {
        return (!this.archived &&
            !this.joined &&
            this.permissionsFor(this.client.user)?.has(this.type === ChannelType.PrivateThread ? PermissionFlagsBits.ManageThreads : PermissionFlagsBits.ViewChannel, false));
    }
    /**
    ...
    get sendable() {
        ...
        if (permissions.has(PermissionFlagsBits.Administrator, false))
            return true;
        return (!(this.archived && this.locked && !this.manageable) &&
