...
'use strict';
const { parse } = nodepath;
const { ChannelType, RouteBases, Routes } = v10;
...
function discordSort(collection) {
    ...
    return collection.toSorted(isGuildChannel
...
function cleanContent(str, channel) {
    ...
    /<(?:(?<type>@[!&]?|#)|(?:\/(?<commandName>[-_\p{L}\p{N}\p{sc=Deva}\p{sc=Thai} ]+):)|(?:a?:(?<emojiName>[\w]+):))(?<id>\d{17,19})>/gu, (match, type, commandName, emojiName, id) => {
        if (commandName)
            return `/${commandName}`;
        if (emojiName)
            return `:${emojiName}:`;
...
export { flatten };
export { fetchRecommendedShardCount };
export { parseEmoji };
export { resolvePartialEmoji };
export { resolveGuildEmoji };
export { makeError };
export { makePlainError };
...
export { basename };
export { cleanContent };
export { cleanCodeBlockContent };
export { parseWebhookURL };
export { transformResolved };
export { resolveSKUId };
