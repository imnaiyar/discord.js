...
'use strict';
const { ButtonStyle } = v10;
const { token, owner, skuId } = auth;
const { Client, Events, codeBlock, GatewayIntentBits, ActionRowBuilder, ButtonBuilder } = index;
...
client.on(Events.EntitlementUpdate, (oldEntitlement, newEntitlement) => console.log('EntitlementUpdate', oldEntitlement, newEntitlement));
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.commandName === 'test') {
        await interaction.reply({
            content: ':3:3:3',
            components: [
                new ActionRowBuilder().setComponents(new ButtonBuilder().setCustomId('test').setLabel('test').setStyle(ButtonStyle.Premium).setSKUId(skuId)),
            ],
        });
    }
});
client.on(Events.MessageCreate, async (message) => {
    ...
    if (message.author.id !== owner || !message.content.startsWith(prefix))
        return;
    let res;
    try {
