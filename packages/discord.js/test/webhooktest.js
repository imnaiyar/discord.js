...
const { setTimeout: sleep } = promises;
const { GatewayIntentBits } = v10;
const { owner, token, webhookChannel, webhookToken } = auth;
const { Client, MessageAttachment, Embed, WebhookClient } = index;
const client = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages] });
const buffer = l => fetch(l)
