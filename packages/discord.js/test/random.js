...
client.on(Events.GuildCreate, guild => guild.members.fetch().catch(err => console.log(`Failed to fetch all members: ${err}\n${err.stack}`)));
// Fetch all members in a newly available guild
client.on(Events.GuildUpdate, (oldGuild, newGuild) => !oldGuild.available && newGuild.available
...
client.on(Events.MessageReactionAdd, (reaction, user) => {
    if (reaction.message.channelId !== '222086648706498562')
        return;
        ...
        m.channel.messages.fetch(mId).then(rM => {
            for (const reaction of rM.reactions.cache.values()) {
                reaction.users.fetch().then(users => {
                    m.channel.send(`The following gave that message ${reaction.emoji}:\n` +
                        `${users
                            .map(u => u.username)
                            .map(t => `- ${t}`)
                            .join('\n')}`);
